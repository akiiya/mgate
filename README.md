...
AP 热点、NAT fallback、Mihomo 代理、TProxy 透明代理——单文件脚本，无外部依赖。
...
`ap-start` 只管理 `ap0`，使用 `/opt/mgate/run/ap/` 下的隔离配置，不覆盖系统 hostapd/dnsmasq。

> AP + managed 并发（`ap0` + `wlan0` 同时工作）取决于网卡/驱动，需真机验证。
